...
public class BottomNavigationView extends RelativeLayout {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            textActiveSize = array.getDimensionPixelSize(R.styleable.BottomNavigationView_bnv_active_text_size,
                    res.getDimensionPixelSize(R.dimen.bottom_navigation_text_size_active));
            textInactiveSize = array.getDimensionPixelSize(R.styleable.BottomNavigationView_bnv_inactive_text_size,
        ...
        for (int i = 0; i < bottomNavigationItems.size(); i++) {
            ...
            if (isTablet)
                view.setPadding(view.getPaddingLeft(), view.getPaddingTop(), i == currentItem ?
                                textActivePaddingTop : withText ? viewInactivePaddingTop : viewInactivePaddingTopWithoutText,
                        view.getPaddingBottom());
            else
                view.setPadding(view.getPaddingLeft(), i == currentItem ? textActivePaddingTop :
                                withText ? viewInactivePaddingTop : viewInactivePaddingTopWithoutText, view.getPaddingRight(),
                        view.getPaddingBottom());
    ...
    private void onBottomNavigationItemClick(final int itemIndex) {
        ...
        for (int i = 0; i < viewList.size(); i++) {
            if (i == itemIndex) {
                ...
                if (isTablet)
                    BottomNavigationUtils.changeRightPadding(view, withText ? viewInactivePaddingTop :
                            viewInactivePaddingTopWithoutText, viewActivePaddingTop);
                else {
                    BottomNavigationUtils.changeViewTopPadding(view, withText ? viewInactivePaddingTop :
                            viewInactivePaddingTopWithoutText, viewActivePaddingTop);
                }
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    Animator changeBackgroundColor = ViewAnimationUtils.createCircularReveal(backgroundColorTemp,
